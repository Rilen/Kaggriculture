...
from submission_v17_3_a6 import KaggricultureAgentV17 as A6
from collections import Counter

for seed in [42, 48, 50]:
    env = make("kaggriculture", configuration={"episodeSteps": 3000, "randomSeed": seed})
    ag = A6(); steps = env.run([lambda o: ag(o), "submission.py"])
    ...
    fert = acts.get("FERTILIZE", 0)
    print(f"A.6 S{seed}: Score={score:.0f} WATER={water} PLANT={plant} wp={water/max(plant,1):.1f} FERT={fert} FEED={acts.get('FEED',0)} HARVEST={acts.get('HARVEST',0)}")
