            ...
            if worker_id in self.worker_targets:
                ...
                if not is_target_valid(tx, ty):
                    ...
                    if (x, y) == (tx, ty):
                        ...
                        is_deadlock = False
                        if tile is None:
                            has_seed = any(winv.get(s, 0) > 0 for s in seeds)
                            if not has_seed:
                                is_deadlock = True
                        elif isinstance(tile, dict) and "animal" in tile and not tile.get("fed_today", True):
                            if winv.get("WHEAT", 0) == 0:
                                is_deadlock = True
                        ...
                        if is_deadlock:
                            ...
                            # Fallthrough para buscar nova tarefa neste turno (ex: ir ao shed)
                        else:
                            if (tx, ty) in assigned:
                                assigned.add((tx, ty))
