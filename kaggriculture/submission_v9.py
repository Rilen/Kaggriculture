"""
Kaggriculture Autonomous AI Agent — submission_v9.py
Experimento 3 (acumulo): v8 (MELAO CEDO + MAIS MAOS) + 3o QUADRANTE.

Base: submission_v8.py. Mudancas sobre o v8:
5. BUY_LAND: 3o quadrante (quads 2->3) quando caixa >= 3000 E saturação
   produtiva real (>= 75% dos tiles ocupados). O 2o quadrante segue como antes.

Herda do v8: MELON_TARGET 24, _choose_plant prioriza MELON, sementes melao 8,
HIRE teto 12. Tudo o mais identico ao v2 (100% plantacao).
"""
...
class GranjaAgent:
    ...
    def _market(self, obs, farm, shed, seeds, day, counts):
        ...
        # [v8] mais maos (ate 12) para cobrir area + volume de melao
        desired = 6 + min(6, day // 4)
        ...
        # --- BUY_LAND: expansao densa (2 quadrante cedo, 3o quando saturar) ---
        if quads == 1 and money >= 1600:
            orders.append(["BUY_LAND"]); money -= 1000
        # [v9] 3o quadrante: so com caixa alta + saturacao produtiva real
        elif quads == 2 and money >= 3000 and counts.get("saturated", False):
    ...
    def _count(self, farm, shed, day):
        ...
        # [v9] saturacao produtiva (necessidade real de expandir para o 3o quad)
        tiles_total = 0
        tiles_used = 0
        for row in tiles:
            for t in row:
        ...
        saturated = tiles_total > 0 and tiles_used >= int(tiles_total * 0.75)
        return {
