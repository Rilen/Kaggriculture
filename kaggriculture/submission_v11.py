"""
GranjaAgent v11 — rota open-loop de pecuaria (8 COW / 4 SHEEP / NE+NW+SW / premium market lead).

Base: V16-RC5 do kernel 'boatlee/v16-rc5-high-score-8c-4s-premium-market-lead'
(rota 8C/4S reconstruida dos replays de Nikita Lugovoy, submission 55440039).
Diferenca vs v10 (C95): market layer com leitura da demanda do TOWN (shops) e
front-run de 1 turno nas vendas premium (MELON/MILK/STRAWBERRY/WOOL) — 'quem vende
antes no mercado compartilhado pega preco melhor'.

Validado localmente (engine 1.32.6 = replays oficiais):
  - Head-to-head vs v10/C95 (48 jogos, 24 seeds x 2 seats): 38-10 (~79% win) — decisivo.
  - Bench local (12 seeds vs passivos): ~150-163k (mesmo nivel do v10).
  - Pool contrafactual (7 replays reais x 2 seats = 14 jogos): 13/14, margem media +83.579.
  - Stress 20 seeds vs pass: media ~149k (min 101k, max 190k), zero erros.
"""
import base64
import copy
import json
