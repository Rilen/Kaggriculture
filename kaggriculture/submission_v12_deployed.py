"""
GranjaAgent v12 — v11 + cash-flow fix no front-run (SELL antes de BUY no market queue).

Base: v11 (V16-RC5 8C/4S + premium market lead), com fix na camada de mercado:
quando o _front_run cria um NOVO SELL para front-run de 1 turno, ele agora é
INSERIDO antes do primeiro BUY na fila de mercado (em vez de append no final),
para que os fundos da venda fiquem disponíveis para os BUY/HIRE subsequentes
daquele mesmo turno (lição do Closer Cleo / reference agents: sells fund buys).

Validado localmente (engine 1.32.7):
  - v11: 20/20 wins vs reference agents T6-T9 (avg margin +24k-25k).
  - v12: H2H vs v11 + reference agents (a validar nesta sessão).
"""
import base64
import copy
import json
