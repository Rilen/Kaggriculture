"""
GranjaAgent v10 — rota open-loop de pecuaria (9 COW / 4 SHEEP / NE+NW+SW / 10-12 maos).

Base: agente topo C95 extraido do kernel 'raykkretzschmar/kaggriculture-findings-from-zero-to-top-meta'
(rota Lev Neganov episode 91587143 player 1 + controller c17/c27 + 3 camadas de weed repair + overlay C94).
Validado localmente (engine 1.32.6 = replays oficiais):
  - 20 seeds vs pass: media ~143k (min 103k, max 178k), zero erros/colapsos.
  - Pool contrafactual (7 replays reais x 2 seats = 14 jogos): 13/14 vitorias, margem media +81.886.
  - Head-to-head vs GranjaAgent v7: 6-0 (77-178k vs 26-33k).

Substitui o v7 (melao-puro) que ficou obsoleto vs a meta de pecuaria deterministica.
"""
import base64
import copy
import json
