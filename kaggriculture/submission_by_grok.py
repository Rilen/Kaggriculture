"""
Kaggriculture Autonomous AI Agent — Version 17 (v17) — Submission Ready

Base: v15 (Fixed Asset Engine)
Mudanças v17 vs v15:
  1. OPENING: MELON 12 -> MELON 9 + STRAWBERRY 3
  2. WHEAT feed buffer dinâmico (preço real do mercado)
  3. STRAWBERRY em tiles vagos após objetivo de pasture
  4. PICKUP animal gate: apenas se empty_past > 0
  5. BUY_SEED STRAWBERRY conservador (dias 0-15, money > 1200)

Preservado: BFS bidirecional, scanner, targets (8 COW / 6 SHEEP / 14 pastures),
BUY_LAND dia 7/11, HIRE adaptativo, FEED/CARE/HARVEST, State Integrity Layer.
"""
...
class KaggricultureAgentV17:
    ...
    def _market(self, obs, tasks, cows, sheep, pastures, empty_past):
        ...
        if needed > 0 and urgent > 10 and money > cost_est + reserve and hires_today < 4:
        ...
        if day <= 18 and len(orders) < MAX_MARKET_ORDERS:
            strw_have = seeds.get("STRAWBERRY", 0)
            if (strw_have < STRAWBERRY_TARGET
                    and days_left >= 10
                    and money > 800
                    and len(orders) < MAX_MARKET_ORDERS):
                need = min(STRAWBERRY_TARGET - strw_have, 1)
                if money > 100 * need + 600:
                    orders.append(["BUY_SEED", "STRAWBERRY", need])
                    money -= 100 * need

        if day <= 10 and len(orders) < MAX_MARKET_ORDERS:
            melon_have = seeds.get("MELON", 0)
            if melon_have < 4 and money > 600 and day <= 10:
                need = min(4 - melon_have, 2)
                if money > 80 * need + 400:
        ...
        if tile is None:
            ...
            if hour <= 20:
                if seeds.get("MELON", 0) > 0 and day <= 12:
                    return ["PLANT", "MELON"]
                if seeds.get("STRAWBERRY", 0) > 0 and days_left >= 10:
                    return ["PLANT", "STRAWBERRY"]
                if seeds.get("WHEAT", 0) > 0 and day <= 12:
                    return ["PLANT", "WHEAT"]
    ...
    @staticmethod
    def _has_high_value_crops(farm):
        count = 0
        for row in farm.get("tiles", []):
            for tile in row:
                if isinstance(tile, dict) and tile.get("kind") == "PLANT":
    ...
    def __call__(self, obs):
        ...
        def worker_act(worker_id, wpos, winv):
            ...
            def is_target_valid(tx, ty):
                ...
                for cond in self._move_priorities(shed, day, winv, empty_past=empty_past, farm=farm):
...
# =============================================================================
# KAGGLE ENTRY POINT (obrigatório)
# =============================================================================
_agent_instance = KaggricultureAgentV17()

def agent(observation, configuration=None):
    """Entrypoint oficial do Kaggle."""
    return _agent_instance(observation)
