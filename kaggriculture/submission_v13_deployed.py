"""
GranjaAgent v13 — v12 + FERTILIZER no front-run.

Base: v12 (v11 + cash-flow fix), com FERTILIZER adicionado ao _FR_ITEMS.
Motivo (forense de 7 derrotas reais 15/08): TODOS os oponentes vencedores vendem
FERTILIZER antes de nós (1a venda step 43-47 vs nossa step 48-49) e vendem 1.735-2.019
unidades vs nossas 300 (Família A). Fertilizante = "free money" dos animais, sem gate
de town demand (_town_demand_now sempre 0), preço base 100, glut lenta (493 até floor).
Front-run de 1 turno nas vendas de FERTILIZER captura preço melhor antes da glut.

Validado localmente (engine 1.32.7):
  - v12: 20/20 wins vs v11 (avg +292), 20/20 vs ref agents T6-T9 (~+25k).
  - v13: H2H vs v12 + ref agents (a validar nesta sessão).
"""
import base64
import copy
import json
...
_FR_ITEMS = ('MELON', 'MILK', 'STRAWBERRY', 'WOOL', 'FERTILIZER')
_FR_STATE = {
...
def _front_run(action, obs, state, step):
    ...
    for item in _FR_ITEMS:
        ...
        if existing is not None:
            ...
            market.insert(_first_buy_slot(market), ["SELL", item, quantity])
        else:
            continue
        action["market"] = market[:10]
