...
class KaggricultureAgentV17:
    ...
    # -------------------------------------------------------------------------
    # v17.3-A.2 — Semantic Task Claiming / Target Validation
    # -------------------------------------------------------------------------
    def validate_task(self, task_name, tile, winv, shed, seeds, day, hour, empty_past=0):
        inv_sum = sum(winv.values())
        days_left = 29 - day
        
        if task_name == "FEED":
            if not isinstance(tile, dict) or tile.get("kind") != "PASTURE" or not tile.get("animal") or tile.get("fed_today"):
                return "INVALID"
            if winv.get("WHEAT", 0) > 0: return "VALID"
            if shed.get("WHEAT", 0) > 0: return "RESUPPLY_REQUIRED"
            return "INVALID"
            
        elif task_name == "HARVEST":
            if not isinstance(tile, dict): return "INVALID"
            if inv_sum > 8: return "INVALID"
            if tile.get("yield_units", 0) > 0: return "VALID"
            if tile.get("kind") == "PLANT":
                crop = tile.get("crop", "")
                age = day - tile.get("planted_day", day)
                if age >= CROPS.get(str(crop), {}).get("max", 99): return "VALID"
            return "INVALID"
            
        elif task_name == "CARE":
            if not isinstance(tile, dict) or tile.get("kind") != "PASTURE" or not tile.get("animal") or tile.get("cared_today"):
                return "INVALID"
            return "VALID"
            
        elif task_name == "COLLECT_FERTILIZER":
            if not isinstance(tile, dict) or tile.get("kind") != "PASTURE" or not tile.get("fertilizer_available"):
                return "INVALID"
            if inv_sum > 8: return "INVALID"
            return "VALID"
            ...
            if not isinstance(tile, dict) or tile.get("kind") != "PASTURE" or tile.get("animal") is not None:
                return "INVALID"
            if winv.get("COW", 0) > 0 or winv.get("SHEEP", 0) > 0: return "VALID"
            if empty_past > 0:
                if shed.get("COW", 0) > 0 or shed.get("SHEEP", 0) > 0: return "RESUPPLY_REQUIRED"
            return "INVALID"
            
        elif task_name == "BUILD_PASTURE":
            if tile is not None: return "INVALID"
            animal_in_shed = shed.get("COW", 0) + shed.get("SHEEP", 0)
            if animal_in_shed > 0 and empty_past == 0 and day <= 15:
                return "VALID"
            return "INVALID"
            ...
            if tile is not None: return "INVALID"
            if hour <= 20:
                if seeds.get("STRAWBERRY", 0) > 0 and days_left >= STRAWBERRY_MIN_DAYS_LEFT: return "VALID"
                if seeds.get("MELON", 0) > 0 and day <= 12: return "VALID"
                if seeds.get("WHEAT", 0) > 0 and day <= 8: return "VALID"
            return "INVALID"
            
        elif task_name == "DIG":
            if not isinstance(tile, dict) or tile.get("kind") != "WEED": return "INVALID"
            return "VALID"
            
        elif task_name.startswith("RESUPPLY_"):
            item = task_name.split("_", 1)[1]
            if winv.get(item, 0) > 0: return "INVALID"
            if shed.get(item, 0) > 0: return "VALID"
            return "INVALID"
            
        return "INVALID"

    # -------------------------------------------------------------------------
    # _move_priorities — v17.3-A.2
    # -------------------------------------------------------------------------
    def _move_priorities(self, shed, day, inv, empty_past=0):
        ...
        return [
            # 1. FEED
            ("FEED", lambda t, x, y: (isinstance(t, dict) and t.get("kind") == "PASTURE"
                             and t.get("animal") and not t.get("fed_today")
                             and (x, y) not in self.fed_this_day)),
            # 2. HARVEST
            ("HARVEST", lambda t, x, y: (isinstance(t, dict)
            ...
            # 3. CARE
            ("CARE", lambda t, x, y: (isinstance(t, dict) and t.get("kind") == "PASTURE"
                             and t.get("animal") and not t.get("cared_today")
                             and (x, y) not in self.cared_this_day)),
            # 4. FERT
            ("COLLECT_FERTILIZER", lambda t, x, y: (isinstance(t, dict) and t.get("kind") == "PASTURE"
                             and t.get("fertilizer_available"))),
            # 5. WATER
            ("WATER", lambda t, x, y: (isinstance(t, dict) and t.get("kind") == "PLANT"
                             and not t.get("watered_today")
                             and (x, y) not in self.watered_this_day)),
            # 6. PLACE animal
            ("PLACE", lambda t, x, y: (isinstance(t, dict) and t.get("kind") == "PASTURE"
                             and t.get("animal") is None)),
            # 7. Cirurgia B — EMERGENCY BUILD PASTURE:
            ("BUILD_PASTURE", lambda t, x, y: (
                t is None
            ...
            ("PLANT", lambda t, x, y: t is None),
            # 9. WEED
            ("DIG", lambda t, x, y: isinstance(t, dict) and t.get("kind") == "WEED"),
        ]
    ...
    def __call__(self, obs):
        ...
        def worker_act(worker_id, wpos, winv):
            ...
            if self._is_shed_adj((x, y)):
                active_resupply = None
                if worker_id in self.worker_targets:
                    tx, ty, tname = self.worker_targets[worker_id]
                    if tname.startswith("RESUPPLY_"):
                        active_resupply = tname.split("_", 1)[1]

                for a in ("COW", "SHEEP"):
                    # MUDANCA 4: apenas PICKUP se ha pastagem vazia disponivel ou eh nosso target
                    if shed.get(a, 0) > 0 and inv_sum == 0 and (empty_past > 0 or active_resupply == a):
                        pickup = safe_return(["PICKUP", a, 1])
                        if pickup: return pickup
                
                if shed.get("WHEAT", 0) > 0 and winv.get("WHEAT", 0) == 0 and (tasks["feed"] or active_resupply == "WHEAT"):
                    pickup = safe_return(["PICKUP", "WHEAT", min(3, shed["WHEAT"])])
                    if pickup: return pickup
                    
                if inv_sum > 3 or (inv_sum > 0 and active_resupply in ("COW", "SHEEP")):
                    return ["DROP"]

            if worker_id in self.worker_targets:
                tx, ty, task_name = self.worker_targets[worker_id]
                target_tile = self._tile_at(farm, (tx, ty))
                
                validity = "INVALID"
                if target_tile != "LOCKED":
                    validity = self.validate_task(task_name, target_tile, winv, shed, seeds, day, hour, empty_past)
                ...
                if validity == "INVALID":
                    ...
                    if not task_name.startswith("RESUPPLY_"):
                        assigned.add((tx, ty))
                    ...
                    if direction:
                        ...
                        if (x, y) == (tx, ty):
                            # Already at target, validate_task says VALID but action was not taken yet
                            # This means we are WAITABLE or waiting for an action to resolve next turn
                            self.worker_failures[worker_id] = 0
                            self.telemetry["target_persistence_turns"] += 1
                            return ["PASS"]
                        else:
            ...
            for task_name, cond in self._move_priorities(shed, day, winv, empty_past=empty_past):
                tx, ty, direction = self._bfs((x, y), cond, farm, assigned)
                if direction:
                    target_tile = self._tile_at(farm, (tx, ty))
                    val = self.validate_task(task_name, target_tile, winv, shed, seeds, day, hour, empty_past)
                    
                    if val == "INVALID" or val == "WAITABLE":
                        continue # Skip to next priority
                        
                    if val == "RESUPPLY_REQUIRED":
                        item = "WHEAT" if task_name == "FEED" else ("COW" if shed.get("COW",0) > 0 else "SHEEP")
                        shed_pos = (len(farm["tiles"][0])//2, len(farm["tiles"])//2)
                        _, _, shed_dir = self._bfs((x, y), lambda t, cx, cy: (cx, cy) == shed_pos, farm, assigned)
                        if shed_dir:
                    ...
                    self.worker_targets[worker_id] = (tx, ty, task_name)
                    self.telemetry["target_claims"] += 1
                    assigned.add((tx, ty))
                    return [direction]
