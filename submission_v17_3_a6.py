...
class KaggricultureAgentV17:
    ...
    def _decide(self, tile, shed, seeds, day, inv, pos, hour, cows, sheep, empty_past, total_crops=0):
        ...
        if tile is None:
            ...
            # Casos 2 e 3: PLANT prioritario (max 50 plants, any hour)
            if total_crops < 50:
        ...
        if isinstance(tile, dict) and tile.get("kind") == "PLANT":
            ...
            # A.6: WATER first, FERTILIZE mid-growth, HARVEST at yield or max
            if not watered:
                return ["WATER"]
            if inv.get("FERTILIZER", 0) > 0 and crop in ("STRAWBERRY", "MELON"):
                return ["FERTILIZE"]
